document.addEventListener("DOMContentLoaded", async () => {
  ...
  function getProductDetails() {
    ...
    function getProductcoments(productID) {
      const comentarios = "https://japceibal.github.io/emercado-api/products_comments/" + productID + ".json";
      const divComentarios = document.getElementById('product-comment');
    
      fetch(comentarios)
        .then(response => {
          if (!response.ok) {
            throw new Error('No se pudo obtener los comentarios de la API');
          }
          return response.json();
        })
        .then(comentarios => {
          // Manipular los datos obtenidos (en este ejemplo, asumimos que los comentarios son un array)
          comentarios.forEach(comentario => {
            const comentarioElement = document.createElement('p');
            comentarioElement.textContent = comentario.text;
            divComentarios.appendChild(comentarioElement);
          });
        })
        .catch(error => {
